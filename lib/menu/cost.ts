...
// Goods cost (sen) for each product id: packaging-type always-included items,
// plus each ingredient entry in the product's merged recipe (global
// always-included steps like ice + category base + the product's own recipe).
// Editing a cost item changes these figures going forward only — orders
// snapshot cost at sale time.
//
// Reads products.recipe, categories.recipe and cost_items. cost_items is
// admin-only under RLS: pass an admin (service-role) client when the caller
// isn't an admin (e.g. at checkout).
export async function getProductCosts(
  ...
  const [items, prods] = await Promise.all([
    db.from("cost_items").select("id, price, is_always_included, is_archived, prep_template"),
  ...
  // Each product's cost is its inherited base (global always-included steps +
  // its category base recipe) merged with its own recipe (see mergeRecipe):
  // inherited ingredients count once, per-drink exclude/override apply,
  // duplicates resolve to the base.
  const categoryIds = [
    ...new Set(
      (prods.data ?? [])
        .map((p) => p.category_id)
  ...
  for (const id of productIds) {
    ...
    const merged = mergeRecipe(inheritedBase, p?.recipe ?? null);
