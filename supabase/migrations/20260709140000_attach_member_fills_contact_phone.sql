-- When staff attach a member, also seed the order's contact_phone from the
-- member's profile IF the order has none yet — so the WhatsApp ready-notice can
-- reach the member (same flow as a number entered at checkout). Never overwrite
-- a number the customer already provided. Applies to both the staff-role attach
-- RPC and the store-account variant.
